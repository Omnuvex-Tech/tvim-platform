        ...
        <div className="pointer-events-none fixed top-4 left-4 right-4 z-[9999] flex flex-col gap-2.5 sm:left-auto sm:right-4">
            {items.map((item) => (
                <div
                    ...
                    role="alert"
                    // Hovering holds the toast open, so there is time to act on it.
                ...
                >
                    ...
                    {item.onClick ? (
                        <button
                            type="button"
                            className="min-w-0 flex-1 cursor-pointer pr-1 text-left"
                            onClick={() => {
                                item.onClick?.();
                                dismiss(item.id);
                            }}
                        >
                            <p className="text-sm leading-5 font-medium text-current">{item.message}</p>
                        </button>
                    ) : (
